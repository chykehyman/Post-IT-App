...
/**
 * Create login to store token and user ID details 
 * @param {obj} auth
 * @returns {obj} object variable to hold token and user ID details
 */
    ...
    describe('SIGNUP OPERATIONS', () => {
        beforeAll((done) => {
            Users.destroy({ where: {} }, { truncate: true }).then((destroyed) => {
                if (destroyed) {
                    console.log('Done truncating users table');
                }
                done();
            });
        });
